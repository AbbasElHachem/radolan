...
class HDF5(object):
    def __init__(self,
                 infile_def=None,
                 infile=None,
                 basepath=r'X:\staff\elhachem\ClimXtreme',
                 rwmode='r',
        ...
        iagg_dt = (pd.to_datetime(self.f.root.timestamps.isoformat[1].decode("utf-8"))
                   - pd.to_datetime(self.f.root.timestamps.isoformat[0].decode("utf-8")))
        if iagg_dt.days == 1:
            self.agg_in = 1440
        else:
            #integer (minutes)
...
# #         metadata = dict(zip(
# #             nodes_meta, [self.f.get_node('/', value)[idxs]
# #                          for value in nodes_meta]))
# #         nodes_meta1 = ['start_idx', 'end_idx']
#         metadata1 = dict(zip(
#             nodes_meta1, [self.f.get_node('/timestamps', value)[idxs]
#                           for value in nodes_meta1]))
#         start_time = [self.f.root.timestamps.isoformat[item]
#                       for item in metadata1['start_idx']]
#         end_time = [self.f.root.timestamps.isoformat[item]
...
#         return metadata
    ...
    def get_pandas_dataframe(self, ids=None, idxs=None, **kwargs):
        ...
        # used
        # self.get_metadata(ids=ids, idxs=idxs)['id']
        hdf5_ids = [ids]
    ...
    def get_pandas_dataframe2(self, ids=None, idxs=None, **kwargs):
        """create pandas dataframe
        ----------
        Keyword Arguments: series, start, end
        ...
        # have same id. But as ids are unique, the id of the hdf5 have to be
        # used
#         hdf5_ids = self.get_metadata(ids=ids, idxs=idxs)['name']
        # if isinstance(hdf5_ids, str):
        #hdf5_ids = [hdf5_ids]
        hdf5_ids = [ids]
...
if __name__ == '__main__':
    ...
    # os.path.dirname(os.path.dirname(os.etcwd()))))
    #     import e_stats
    ##    import e_stats_compare
    ##    fig, ax = plt.subplots(1)

    ##
    pass
    HDF52 = HDF5(
        infile=r"X:\hiwi\ElHachem\Jochen\Reutlingen_Radolan\dataframe_as_HDF5_Reutlingen_Stations\Reutlingen_12pluvios_215052014_23062020.h5")
